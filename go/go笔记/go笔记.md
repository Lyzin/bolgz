...
>
> - 注意：
>   - 用短变量方式直接定义数组，但是必须要对数组进行初始化，否则会下面的错误
>   - 错误提示：`./main.go:7:8: type []int is not an expression`
>   - 只有关键字`var`可以用来声明数组，可以先对数组元素初始化，其他方式都需要对数组进行初始化
...
> 可以由数组得到切片，和`python`的切片操作一致
>
> 切片`s1[0:4]`表示`左包含右不包含`,也就是顾头不顾腚，左闭右开
...
>
> 由此可以看出切片是引用类型，只要修改了切片的某个值，原始数组的对应位置元素也会进行同步修改
